...
class SlowbarScreen extends StatefulWidget {
  const SlowbarScreen({super.key});

  @override
  State<SlowbarScreen> createState() => _SlowbarScreenState();
}

class _SlowbarScreenState extends State<SlowbarScreen> {
  ...
  Future<void> fetchDataStore() async {
    try {
      ...
      if (res.statusCode == 200) {
        ...
        result = dataStore
            .where((user) =>
                user["type"].toString().toLowerCase().contains("slowbar"))
            .toList();
        setState(() {
          _foundData = result;
        });
