...
class HybridbarScreen extends StatefulWidget {
  const HybridbarScreen({super.key});

  @override
  State<HybridbarScreen> createState() => _HybridbarScreenState();
}

class _HybridbarScreenState extends State<HybridbarScreen> {
  ...
  Future<void> fetchDataStore() async {
    try {
      ...
      if (res.statusCode == 200) {
        ...
        result = dataStore
            .where((user) =>
                user["type"].toString().toLowerCase().contains("hybrid"))
            .toList();
        setState(() {
          _foundData = result;
        });
